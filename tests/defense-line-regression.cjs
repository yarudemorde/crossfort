...
(async()=>{
  ...
  ctx.state.playerBoard[1]=unit();ctx.state.playerBoard[1].canAttack=true;await ctx.attack('player',1);check(ctx.state.enemyDefense[0]===2&&ctx.state.enemyDefense[1]===1&&ctx.state.enemyLife===15,'different lanes progress independently and each intact wall blocks once');
  ...
  reset();await ctx.triggerCard(unit('帝国の擲弾兵'),'onPlay','player',0,{});check(ctx.state.enemyLife===19&&ctx.state.enemyDefense[0]===0,'Imperial Grenadier effect damage bypasses intact defense and does not advance it');
  reset();const cpu=unit();cpu.canAttack=true;ctx.state.enemyBoard[2]=cpu;for(let hit=1;hit<=3;hit++){cpu.canAttack=true;const before=ctx.state.playerLife;await ctx.executeAttackAction({side:'enemy',lane:2,destination:2,card:cpu});check(before-ctx.state.playerLife===(hit===1?0:hit===2?2:3)&&ctx.state.playerDefense[2]===Math.min(2,hit),'CPU uses the same line rule on hit '+hit)}
  reset();let onePower=unit();onePower.power=1;onePower.canAttack=true;ctx.state.playerBoard[0]=onePower;await ctx.attack('player',0);check(ctx.state.enemyLife===20&&ctx.state.enemyDefense[0]===1,'one-power direct attack is fully blocked but cracks intact wall');
  reset();let tenPower=unit();tenPower.power=10;tenPower.canAttack=true;ctx.state.playerBoard[0]=tenPower;await ctx.attack('player',0);check(ctx.state.enemyLife===20&&ctx.state.enemyDefense[0]===1,'ten-power direct attack is fully blocked but cracks intact wall');
  reset();let castle=unit('クロスフォート城');ctx.state.enemyLandmarks[0]=castle;ctx.state.playerBoard[0]=unit();ctx.state.playerBoard[0].canAttack=true;await ctx.attack('player',0);check(ctx.state.enemyDefense[0]===1&&ctx.state.enemyLandmarks[0]===castle&&ctx.state.enemyLife===20,'intact wall blocks combat and preserves same-lane landmark');ctx.state.playerBoard[0].canAttack=true;await ctx.attack('player',0);check(ctx.state.enemyDefense[0]===2&&ctx.state.enemyLandmarks[0]===null&&ctx.state.enemyLife===18,'cracked wall passes combat damage and destroys same-lane landmark');
  ...
  reset();ctx.state.playerBoard[1]=unit('帝国の騎馬隊');ctx.state.playerBoard[1].power=4;ctx.state.playerBoard[1].canAttack=true;ctx.state.enemyBoard[1]=unit('群れ呼び蜥蜴');await ctx.attack('player',1);check(ctx.state.enemyDefense[1]===1&&ctx.state.enemyLife===20,'intact wall blocks piercing combat damage and cracks');
  reset();ctx.state.enemyDefense[0]=2;let breached=unit();breached.power=3;breached.canAttack=true;ctx.state.playerBoard[0]=breached;await ctx.attack('player',0);check(ctx.state.enemyLife===16&&ctx.state.enemyDefense[0]===2,'breached wall adds fixed one to three combat damage');
  reset();ctx.state.enemyDefense[0]=2;let doubled=unit();doubled.power=3;doubled.canAttack=true;ctx.state.playerBoard[0]=doubled;ctx.state.playerBoard[4]=unit('反乱の先導者ボザヤ');await ctx.attack('player',0);check(ctx.state.enemyLife===13&&ctx.state.enemyDefense[0]===2,'breached wall adds one after three-power combat is doubled: 3x2+1');
  reset();ctx.revengeCalls=[];run("originalResolveRevenge=resolveRevenge;resolveRevenge=async function(side,damage){revengeCalls.push([side,damage])}");let revengeAttack=unit();revengeAttack.power=3;revengeAttack.canAttack=true;ctx.state.playerBoard[0]=revengeAttack;await ctx.attack('player',0);check(ctx.revengeCalls.length===0&&ctx.state.enemyDefense[0]===1,'blocked combat does not trigger revenge');revengeAttack.canAttack=true;await ctx.attack('player',0);check(ctx.state.enemyLife===17&&ctx.state.enemyDefense[0]===2&&ctx.revengeCalls.length===1&&ctx.revengeCalls[0][1]===3,'cracked wall passes three damage, breaches, and triggers revenge with actual damage');run('resolveRevenge=originalResolveRevenge');
